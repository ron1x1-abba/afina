...
public:
    ...
    void Configure(const cxxopts::Options &options) {
        ...
        if (storage_type == "st_lru") {
            ...
            std::unique_ptr<Afina::Backend::StripedLRU> u_p = Afina::Backend::StripedLRU::BuildStripedLRU(1024 * 1024 * 1024 , 16);
            storage = std::shared_ptr<Afina::Backend::StripedLRU>(std::move(u_p));
        } else {
